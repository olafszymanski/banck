<template>
  <div :class="`flex w-full px-4 py-2 ${ state.trend ? 'bg-light-green' : 'bg-light-rose' } ${ state.error ? 'bg-light-rose' : '' } rounded-10px`">
    <div class="flex flex-row xs:flex-row 2lg:flex-col 1.5xl:flex-row justify-between xs:items-center lg:items-start 1.5xl:items-center w-full pl-3 xs:pl-2">
      ...
      <div v-if="!state.error" :class="`flex xs:justify-center items-center ${ state.trend ? 'text-strong-green' : 'text-strong-rose' }`">
...
export default {
  ...
  setup(props) {
    const token = 'sandbox_c1qag2aad3iai6qu0g1g'
