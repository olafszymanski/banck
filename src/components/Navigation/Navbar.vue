<template>
    ...
    <div class="w-11/12 sm:w-10/12 my-auto">
      <div class="flex flex-row justify-end sm:justify-between items-center">
        <div class="flex sm:flex-none flex-grow my-auto justify-center">
          <span class="text-maximum-red text-25px xs:text-35px font-bold cursor-pointer" @click="logoClicked">B<span class="text-chinese-black text-20px xs:text-30px font-bold">anck</span></span>
        </div>
        <Menu class="hidden sm:flex flex-row space-x-5 my-auto" :deactivateMenu="deactivateMenu" :key="updateMenu"/>
  ...
  <transition name="slide">
    <div v-if="menuActive" class="absolute flex justify-center w-screen my-40px xs:my-65px md:my-90px bg-white rounded-b-10px shadow-lg sm:invisible z-10">
      <Menu class="flex flex-col space-y-5 w-4/5 p-6 my-10px text-center" :deactivateMenu="deactivateMenu" :key="updateMenu"/>
    </div>
  </transition>
</template>
